...
- 今回のスプリントではどれだけの SP を消化できたかを知りたかった
- プロジェクトの合計 StoryPoint を消化するためには、あと何スプリント必要なのかを知りたかった
- [Jira の課題を Excel に出力するプラグイン](https://marketplace.atlassian.com/apps/1221301/jira-cloud-for-excel-official?tab=overview&hosting=cloud)はあるが、Excel を頑張るよりも、API で取得して TypeScript でいじった方が簡単そうだった
...
Jira の API は[jira-client](https://www.npmjs.com/package/jira-client)で叩きます。
...
これらのフィールド名は、「自分のプロジェクト」だけでなく、「社内の他のプロジェクトが設定したフィールド」も含まれています。
つまり、社内で Jira の使い方が統一されていない場合は、自分のプロジェクトには不要なフィールドが大量に紛れ込んでくるということです。

自分のプロジェクトで使っていないフィールドの値は `null` になっています。

`issues` の中に `fields` があって、`fields` が持つ `parent` が `issue` と同じ型で入れ子になっていたりと、レスポンスはそのままだと非常に使いにくい状態になっています。

なので、型を自分で定義して、レスポンスを型にはめていくような作業をするとデータの利用が楽になります。

自社の情報が入らないように抜粋すると、こんな感じの型を作ります。
